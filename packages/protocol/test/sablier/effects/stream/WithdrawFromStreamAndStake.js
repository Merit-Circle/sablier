      ...
      describe("when the withdrawal amount does not exceed the available balance", function () {
        describe("when the balance is not withdrawn in full", function () {
          ...
          it("withdraws from the stream", async function () {
            ...
            const depositsOf = await this.timeLockPool.depositsOf.call(this.recipient, 0);
            BigNumber(depositsOf.amount).should.be.bignumber.equal(STANDARD_SALARY.dividedBy(2));

            const timelockBalance = await this.timeLockPool.balanceOf(this.recipient);
            BigNumber(timelockBalance).should.be.bignumber.equal(STANDARD_SALARY.dividedBy(2));
          });
        ...
        describe("when the balance is withdrawn in full", function () {
          ...
          it("withdraws from the stream", async function () {
            ...
            const depositsOf = await this.timeLockPool.depositsOf.call(this.recipient, 0);
            BigNumber(depositsOf.amount).should.be.bignumber.equal(STANDARD_SALARY);

            const timelockBalance = await this.timeLockPool.balanceOf(this.recipient);
            BigNumber(timelockBalance).should.be.bignumber.equal(STANDARD_SALARY);
          });
...
function shouldBehaveLikeERC1620WithdrawFromStreamAndStake(alice, bob, eve) {
...
module.exports = shouldBehaveLikeERC1620WithdrawFromStreamAndStake;
